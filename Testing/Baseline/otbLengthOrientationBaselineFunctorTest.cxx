...
int otbLengthOrientationBaselineFunctorTest(int argc, char * argv[])
{
  ...
  if(std::abs(baseline->GetLengthBaseline() - resultLength) > 0.0 )
  {
	std::cout << "LengthBaseline : " << baseline->GetLengthBaseline() << std::endl;
	std::cout << "Expected value : " << resultLength << std::endl;
    return EXIT_FAILURE;
  }

  if(std::abs(baseline->GetOrientationBaseline() -RTNBaseline[2]) >0.0)
  {
